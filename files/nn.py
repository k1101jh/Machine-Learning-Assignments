...
def AffineBackward(grad_y, x, w):
    ...
    grad_x = np.dot(grad_y, w.transpose())
    grad_w = np.dot(x.transpose(), grad_y)
...
def ReLUBackward(grad_y, x, y):
    ...
    grad_x = np.where(y > 0.0, grad_y, 0.0)
    return grad_x
...
def NNUpdate(model, eps, momentum):
    ...
    model['mW1'] = momentum * model['mW1'] - eps * model['dE_dW1']
    model['mW2'] = momentum * model['mW2'] - eps * model['dE_dW2']
    model['mW3'] = momentum * model['mW3'] - eps * model['dE_dW3']
    ...
    # model['W1'] = model['W1'] - model['mW1']
    # model['W2'] = model['W2'] - model['mW2']
    # model['W3'] = model['W3'] - model['mW3']
...
def Train(model, forward, backward, update, eps, momentum, num_epochs,
    ...
    test_ce, test_acc = Evaluate(
        inputs_test, target_test, model, forward, batch_size=batch_size)
    Error_str = 'CE: Train {:.5f} Validation {:.5f} Test {:.5f}'.format( \
          train_ce, valid_ce, test_ce)
    Acc_str = 'Acc: Train {:.5f} Validation {:.5f} Test {:.5f}'.format( \
    ...
    if mode == 1 or mode == 2 :
        if not os.path.exists('figure/problem' + str(mode)):
            os.makedirs('figure/problem' + str(mode))
        
        f = open('figure/problem' + str(mode) + '/result.txt', 'a')

        f.write('\nLR : '+ str(eps) + ' Momentum : ' + \
        str(momentum) + ' Batch_size : ' + str(batch_size))
    elif mode == 3:
        if not os.path.exists('figure/problem' + str(mode)):
            os.makedirs('figure/problem' + str(mode))
        ...
        f.write('\nLR : '+ str(eps) + ' Hidden unit : [' + \
        str(model['W2'].shape[0]) + ',' + str(model['W2'].shape[1]) + \
        '] Batch_size : ' + str(batch_size))
        Best_error_epoch = test_ce_list.index(min(test_ce_list))
        print(min(test_acc_list))
        print(Best_error_epoch)
        f.write('\n\tBest Test Error: {:.3f}'.format(test_ce_list[Best_error_epoch]) + \
            ' Test Acc: {:.3f}'.format(test_acc_list[Best_error_epoch]) + \
                ' Epoch: ' + str(Best_error_epoch))
    f.write('\n\t' + Error_str)
    f.write('\n\t' + Acc_str)
    f.close()

    DisplayPlot(train_ce_list, valid_ce_list, 'Cross Entropy', \
        save_name='figure/problem' + str(mode) + '/' + save_name + '_E', number=0)
    DisplayPlot(train_acc_list, valid_acc_list, 'Accuracy', \
        save_name='figure/problem' + str(mode) + '/' + save_name + '_A', number=1)
...
def main():
    ...
    if mode == 1:
        ...
        batch_size = 400
        num_epochs = 2000
        hidden_unit_num_list = [[x, y] for x in [10, 50, 100] \
                                    for y in [10, 50, 100]]

        for num_hiddens in hidden_unit_num_list:
            save_name = 'hidden_[' + str(num_hiddens[0]) + \
                    ',' + str(num_hiddens[1]) + ']'
